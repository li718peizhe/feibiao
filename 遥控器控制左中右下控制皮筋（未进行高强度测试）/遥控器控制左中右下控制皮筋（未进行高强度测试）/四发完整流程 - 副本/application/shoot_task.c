...
void shoot_task(void const *pvParameters)
{
    ...
    Shoot_Init();
        ...
        CAN_CmdShoot(ShootControl.give_current[0], ShootControl.give_current[1], ShootControl.give_current[2]);
        vTaskDelay(1);
...
void Shoot_Init(void)
{
    ...
    ShootControl.LeftFireData = get_shoot_measure_point(1);
    ShootControl.RightFireData = get_shoot_measure_point(2);
    ShootControl.TriggerData = get_shoot_measure_point(0);
    ShootControl.RC_data = get_remote_control_point();

    PID_init(&trigger_speed_motor_pid, PID_POSITION, TriggerSpeedPID, BULLET_ANG_PID_MAX_OUT, BULLET_ANG_PID_MAX_IOUT);
    PID_init(&trigger_Angle_motor_pid, PID_POSITION, TriggerAnglePID, BULLET_SPEED_PID_MAX_OUT, BUFFET_SPEED_PID_MAX_IOUT);
